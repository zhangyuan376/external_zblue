		    ...
		    at_finish_cb_t finish, const char *format, ...)
{
	...
	LOG_INF("HF %p, DLC %p sending buf %p", hf, &hf->rfcomm_dlc, buf);
			       ...
			       char *name, uint32_t min, uint32_t max)
{
	...
	LOG_INF("index: %u, name: %s, min: %u, max:%u", index, name, min, max);
		   ...
		   enum at_cme cme_err)
{
	...
	LOG_INF("AT+CLCC (result %d) on %p", result, hf);
...
static void hf_call_state_update(struct bt_hfp_hf_call *call, int state)
{
	...
	LOG_INF("Call %p state update %d->%d", call, old_state, state);
...
static int clcc_handle(struct at_client *hf_at)
{
	...
	LOG_INF("CLCC idx %d dir %d status %d mode %d mpty %d number %s type %d",
				...
				uint32_t value)
{
	...
	LOG_INF("Index :%u, Value :%u", index, value);
...
static int cnum_handle(struct at_client *hf_at)
{
	...
	LOG_INF("CNUM number %s type %d service %d", number, type, service);
		   ...
		   enum at_cme cme_err)
{
	...
	LOG_INF("CMEE set (result %d) on %p", result, hf);
		   ...
		   enum at_cme cme_err)
{
	...
	LOG_INF("COPS set (result %d) on %p", result, hf);
		   ...
		   enum at_cme cme_err)
{
	...
	LOG_INF("CLIP set (result %d) on %p", result, hf);
		   ...
		   enum at_cme cme_err)
{
	...
	LOG_INF("VGM set (result %d) on %p", result, hf);
		   ...
		   enum at_cme cme_err)
{
	...
	LOG_INF("VGS set (result %d) on %p", result, hf);
		   ...
		   enum at_cme cme_err)
{
	...
	LOG_INF("CCWA set (result %d) on %p", result, hf);
...
static int at_cmd_init_start(struct bt_hfp_hf *hf)
{
	...
	while (ARRAY_SIZE(cmd_init_list) > hf->cmd_init_seq) {
		LOG_INF("Fetch AT command (%d)", hf->cmd_init_seq);
		send = cmd_init_list[hf->cmd_init_seq].send;
		if (send) {
			LOG_INF("Send AT command");
...
static void slc_completed(struct at_client *hf_at)
{
	...
	if (bt_hf->connected) {
#if !defined(CONFIG_BT_HFP_HF_HF_INDICATORS)
		bt_hf->codec_changed(hf, BT_HFP_HF_CODEC_CVSD);
#endif /* !CONFIG_BT_HFP_HF_HF_INDICATORS */
		   ...
		   enum at_cme cme_err)
{
	...
	LOG_INF("AT+COPS? (result %d) on %p", result, hf);
		   ...
		   enum at_cme cme_err)
{
	...
	LOG_INF("AT+BINP=1 (result %d) on %p", result, hf);
		   ...
		   enum at_cme cme_err)
{
	...
	LOG_INF("AT+VTS (result %d) on %p", result, hf);
		   ...
		   enum at_cme cme_err)
{
	...
	LOG_INF("AT+CNUM (result %d) on %p", result, hf);
		   ...
		   enum at_cme cme_err)
{
	...
	LOG_INF("AT+BIA (result %d) on %p", result, hf);
		   ...
		   enum at_result result, enum at_cme cme_err)
{
	...
	LOG_INF("AT+BIEV (result %d) on %p", result, hf);
		   ...
		   enum at_cme cme_err)
{
	...
	LOG_INF("ATA (result %d) on %p", result, hf);
		   ...
		   enum at_cme cme_err)
{
	...
	LOG_INF("AT+BTRH (result %d) on %p", result, hf);
		   ...
		   enum at_cme cme_err)
{
	...
	LOG_INF("AT+CHUP (result %d) on %p", result, hf);
		   ...
		   enum at_cme cme_err)
{
	...
	LOG_INF("ATD (result %d) on %p", result, hf);
		   ...
		   enum at_cme cme_err)
{
	...
	LOG_INF("AT+BLDN (result %d) on %p", result, hf);
...
int zblue_bt_hfp_hf_redial(struct bt_hfp_hf *hf)
{
	struct bt_hfp_hf_call *call;
	int err;
		   ...
		   enum at_cme cme_err)
{
	...
	LOG_INF("BCC (result %d) on %p", result, hf);
		   ...
		   enum at_cme cme_err)
{
	...
	LOG_INF("BCS (result %d) on %p", result, hf);
		   ...
		   enum at_cme cme_err)
{
	...
	LOG_INF("BAC (result %d) on %p", result, hf);
		   ...
		   enum at_cme cme_err)
{
	...
	LOG_INF("AT+NREC=0 (result %d) on %p", result, hf);
		   ...
		   enum at_cme cme_err)
{
	...
	LOG_INF("AT+CCWA (result %d) on %p", result, hf);
		   ...
		   enum at_cme cme_err)
{
	...
	LOG_INF("AT+CHLD=0 (result %d) on %p", result, hf);
		   ...
		   enum at_result result, enum at_cme cme_err)
{
	...
	LOG_INF("AT+CHLD=1 (result %d) on %p", result, hf);
		   ...
		   enum at_result result, enum at_cme cme_err)
{
	...
	LOG_INF("AT+CHLD=2 (result %d) on %p", result, hf);
		   ...
		   enum at_result result, enum at_cme cme_err)
{
	...
	LOG_INF("AT+CHLD=3 (result %d) on %p", result, hf);
		   ...
		   enum at_result result, enum at_cme cme_err)
{
	...
	LOG_INF("AT+CHLD=4 (result %d) on %p", result, hf);
		   ...
		   enum at_result result, enum at_cme cme_err)
{
	...
	LOG_INF("AT+CHLD=1<idx> (result %d) on %p", result, hf);
		   ...
		   enum at_result result, enum at_cme cme_err)
{
	...
	LOG_INF("AT+CHLD=2<idx> (result %d) on %p", result, hf);
		   ...
		   enum at_result result, enum at_cme cme_err)
{
	...
	LOG_INF("AT+BVRA=1 (result %d) on %p", result, hf);
		   ...
		   enum at_result result, enum at_cme cme_err)
{
	...
	LOG_INF("AT+BVRA=0 (result %d) on %p", result, hf);
		   ...
		   enum at_result result, enum at_cme cme_err)
{
	...
	LOG_INF("AT+BVRA=2 (result %d) on %p", result, hf);
...
{
	LOG_INF("DLC %p sent cb (err %d)", dlc, err);
}
...
int zblue_bt_hfp_hf_connect(struct bt_conn *conn, struct bt_hfp_hf **hf, uint8_t channel)
{
	printk("zblue_bt_hfp_hf_connect\n");
...
int zblue_bt_hfp_hf_disconnect(struct bt_hfp_hf *hf)
{
	LOG_INF("aaaaaaaaa");
